...
import {
  Activity, ShieldAlert, CheckCircle2, Cpu,
  RefreshCw, Trash2, TrendingUp, Clock
} from 'lucide-react';
...
const getBarColor = (name, index) => {
  return ATTACK_COLOURS[name] || FALLBACK_COLOURS[index % FALLBACK_COLOURS.length];
};
...
function FeatBar({ label, value, max }) {
  ...
  return (
    <div className="feat-bar-row">
      <div className="feat-bar-label" title={label}>{label}</div>
  ...
  return (
    <div className="conf-bar-wrapper">
      <div className="conf-bar-track">
        <div className={`conf-bar-fill ${cls}`} style={{ width: `${value}%` }} />
      </div>
...
export default function Dashboard() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (featRes.status === 'fulfilled' && Array.isArray(featRes.value.features)) {
    ...
    <>
      ...
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p>Real-time network intrusion monitoring · Model v3 (Random Forest)</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button id="btn-refresh" className="btn btn-ghost btn-sm" onClick={fetchAll} disabled={loading}>
            <RefreshCw size={14} className={loading ? 'spinner' : ''} />
            Refresh
          </button>
          <button id="btn-reset" className="btn btn-danger btn-sm" onClick={resetSession} disabled={resetting}>
            <Trash2 size={14} />
      ...
      <div className="page-body">
        ...
        <div className="stats-grid">
          <div className="stat-card cyan">
            <div className="stat-icon cyan"><Activity size={18} /></div>
            <div className="stat-value white">{stats.total.toLocaleString()}</div>
            <div className="stat-label">Total Packets Evaluated</div>
            <div className="stat-sub"><TrendingUp size={11} /> All sessions combined</div>
          </div>

          <div className="stat-card red">
            <div className="stat-icon red"><ShieldAlert size={18} /></div>
            <div className="stat-value red">{stats.attacks.toLocaleString()}</div>
          ...
          <div className="stat-card orange">
            <div className="stat-icon orange"><Cpu size={18} /></div>
            <div className="stat-value orange">{stats.total > 0 ? MODEL_ACC : '–'}%</div>
            ...
            <div className="card-body">
              {feats.length > 0 ? (
                feats.slice(0, 8).map(f => (
                  <FeatBar key={f.feature} label={f.feature} value={f.importance} max={maxFeat} />
                ))
              ) : (
                <div style={{ color: 'var(--text-muted)', fontSize: 13, textAlign: 'center', padding: '24px 0' }}>
                  {loading ? 'Loading…' : 'No feature data — train model first.'}
                </div>
              )}
            </div>
          </div>
              ...
              {timelineData.length > 0 ? (
                <ResponsiveContainer width="100%" height={180}>
                  <AreaChart data={timelineData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="confidence" stroke="var(--accent)" strokeWidth={2} fillOpacity={1} fill="url(#colorConf)" />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
              ...
              {stats.total > 0 ? (
                <ResponsiveContainer width="100%" height={180}>
                  <PieChart>
                    ...
                    >
                      {pieData.map((d) => (
                        <Cell key={d.name} fill={d.name === 'Attack' ? '#ff4757' : '#2ed573'} />
                      ))}
                    </Pie>
        ...
        <div className="card">
          <div className="card-header" style={{ paddingBottom: 14 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Latest 10 events</div>
          </div>
